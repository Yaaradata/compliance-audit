#!/usr/bin/env python3
"""
Truncate and load swift_2025.evidence_based_questions from a CSV file.

Usage:
  python backend/scripts/load_swift_2025_evidence_questions_csv.py
  python backend/scripts/load_swift_2025_evidence_questions_csv.py --file "SWIFT_2025.evidence_based-questions - studio_results_20260326_enriched.csv.csv"
  python backend/scripts/load_swift_2025_evidence_questions_csv.py --no-truncate
"""
            ...
            payload.append(
                {
                    ...
                    "cscf_version": to_text(row["cscf_version"]) or "2025v",
                    "created_at": to_text(row["created_at"]),
                    "guide": to_text(row["guide"]),
                    "show_when_question": to_text(row["show_when_question"]),
                    "show_when_values": to_json_array_text(row["show_when_values"]),
                    "evidence_required_raw": to_text(row["evidence_required_raw"]),
                    "evidence_source": to_text(row["evidence_source"]),
                    "collection_method": to_text(row["collection_method"]),
                    "aws_auto_level": to_text(row["aws_auto_level"]),
...
def run(csv_path: Path, truncate_first: bool) -> int:
    ...
    try:
        with conn.cursor() as cur:
            if truncate_first:
                cur.execute("TRUNCATE TABLE swift_2025.evidence_based_questions RESTART IDENTITY CASCADE")
            ...
            sql = f"INSERT INTO swift_2025.evidence_based_questions ({sql_columns}) VALUES ({sql_values})"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Truncate and load swift_2025.evidence_based_questions from CSV.")
