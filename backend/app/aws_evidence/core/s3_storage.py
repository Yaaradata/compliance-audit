"""Upload and fetch evidence files in Amazon S3 for SWIFT evidence."""
...
def get_s3_client():
    if not boto3:
        raise RuntimeError("boto3 is required. Install it in backend requirements.")
    return boto3.client(
        "s3",
...
def ensure_bucket() -> None:
    ...
    try:
        ...
        except ClientError as e:  # type: ignore[assignment]
        ...
        # Do not block app startup if AWS is unreachable
        return


def s3_key(collector_name: str, timestamp: datetime) -> str:
    """Key format, e.g. aws/123456789012/iam/2026-03-14.json."""
...
def upload_evidence_bytes(body: bytes, key: str) -> tuple[str, str]:
    """Upload raw bytes to S3 at key. Returns (s3_uri, sha256_hex)."""
    client = get_s3_client()
    bucket = config.S3_BUCKET_NAME
