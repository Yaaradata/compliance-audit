...
def collect(region: str, account_id: str, session=None) -> list:
    ...
    results = []
    try:
        _collect_inspector(region, account_id, now, results, session)
    except Exception as e:
        payload = {"collector": COLLECTOR_NAME, "account_id": account_id, "region": region, "collected_at": now.isoformat(), "error": str(e), "finding_report_status": [], "findings_sample": []}
        for item_code, control_id in CONTROL_MAPPINGS:
            results.append((payload, item_code, control_id, EVIDENCE_TYPE, SOURCE_SYSTEM))
    return results


def _collect_inspector(region: str, account_id: str, now: datetime, results: list, session=None) -> None:
    try:
