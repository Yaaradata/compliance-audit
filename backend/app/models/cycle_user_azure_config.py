...
class CycleUserAzureConfig(Base):
    ...
    azure_oauth_state: Mapped[str | None] = mapped_column(String(128), nullable=True)
    azure_oauth_state_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    encrypted_oauth_refresh_token: Mapped[str | None] = mapped_column(Text, nullable=True)
    entra_signin_username: Mapped[str | None] = mapped_column(String(320), nullable=True)
