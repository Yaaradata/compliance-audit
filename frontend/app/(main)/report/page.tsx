...
export default function ReportPage() {
  ...
  useEffect(() => {
    if (!activeCycleId) { setSections([]); setLoading(false); return; }
    setLoading(true);
    api.get<ApiReport[]>(`/assessments/${activeCycleId}/reports`)
      .then((reports) => {
        if (reports.length > 0) {
    ...
    return (
      <div className="text-center py-20 text-gray-400">
        ...
        {activeCycleId && (
          <button
            onClick={() => {
              api.post<ApiReport>(`/assessments/${activeCycleId}/reports`, { report_kind: "draft" })
                .then((r) => {
                  setReportId(r.id);
