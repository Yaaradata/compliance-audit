...
import type {
  FastTagAuditPosture,
  FastTagSelectionSummary,
  FastTagSummaryFinding,
} from './fastTagRegionSummary';
...
export default function FastTagRegionSummaryPanel({ summary, onViewTable }: Props) {
    ...
    <section className={`overflow-hidden rounded-xl bg-white shadow-sm ring-1 ${style.ring}`}>
      ...
      <div className="space-y-6 px-5 py-5">
        ...
        <div className="grid gap-4 lg:grid-cols-5">
          ...
          {/* AI Insights — CRO / FastTag officer memo */}
            ...
            <ul className="mt-3 grid gap-2 sm:grid-cols-2">
              {summary.findings.map((item) => {
                ...
                return (
                  <li
                    key={item.id}
                    className={`rounded-lg p-3 ring-1 ${tone.box}`}
                  >
