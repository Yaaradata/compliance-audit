...
export default function FastTagJourneyCasesView({
  ...
  const filteredCases = useMemo(() => {
    const rows = viewAllIndia
      ? cases
      : filterFastTagCasesByRegion(cases, regionCode || null);
    if (!stageFilter) return rows;
    return rows.filter((kase) => isFastTagStageAuditFinding(kase, stageFilter));
  ...
  const toggleStage = (stageId: string) => {
    const turningOff = stageFilter === stageId;
    if (!regionCode) {
      setViewAllIndia(true);
      setViewAllInRegion(false);
    } else {
      setViewAllIndia(false);
      setViewAllInRegion(turningOff);
    }
