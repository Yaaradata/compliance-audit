/**
 * Persona- and screen-owned insight selection for v6.
 *
 * Rules:
 ...
 */
import type { AIInsightV6 } from "./aiContract";
...
const SEVERITY_RANK: Record<AIInsightV6["severity"], number> = {
  high: 0,
  medium: 1,
  low: 2,
};
 ...
 */
export function insightMatchesPersona(insight: AIInsightV6, persona: string): boolean {
 ...
 */
export function owningScreen(insight: AIInsightV6): string {
...
export function insightFingerprint(insight: AIInsightV6): string {
...
function sortInsights(a: AIInsightV6, b: AIInsightV6): number {
...
export function groupInsightsByType(
  insights: AIInsightV6[],
): { type: string; items: AIInsightV6[] }[] {
  const order: string[] = [];
  const map = new Map<string, AIInsightV6[]>();
