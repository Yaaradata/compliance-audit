/**
 * v6 AI insight contract — one citation model for explorer AND board signals.
...
import { V6_PERSONA_INSIGHTS } from "./personaInsightSeeds";
...
const SEVERITY_MAP: Record<BoardSignal["severity"], AIInsightV6["severity"]> = {
  S1: "high",
  S2: "medium",
  S3: "low",
};
...
/** Map numeric legacy confidence to band — v6 explorer never shows bare %. */
...
export function boardSignalToAiInsight(signal: BoardSignal): AIInsightV6 {
...
type LegacyInsight = {
  ...
  inputsNotSeen?: string[];
  humanActionStatus?: string;
  independenceLineage?: AIInsightV6["independenceLineage"];
  relatedEntityIds?: { type: string; id: string }[];
  derivation?: Derivation;
};

/** Normalise legacy mock insights into the v6 contract. */
export function legacyInsightToV6(insight: LegacyInsight): AIInsightV6 {
  return {
    id: insight.id,
    type: insight.type,
    title: insight.title,
...
/**
 * Explorer feed — detector signals, legacy records, and v6 persona-owned records.
...
export function buildExplorerInsights(
  ...
  const base = legacyInsights.map(legacyInsightToV6);
  const signals = boardSignals.map(boardSignalToAiInsight);
  return [...signals, ...V6_PERSONA_INSIGHTS, ...base];
}

export function findExplorerInsight(
  insights: AIInsightV6[],
  entityId: string,
): AIInsightV6 | undefined {
  return (
...
/** Resolve an insight for the v6 drawer — legacy id or board signal id. */
