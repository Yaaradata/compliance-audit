/**
 * Every evidenceRef that v6 ClaimLines / change rows can surface.
...
import { DOMAIN_EVIDENCE, CRSA_MECHANISM_TAGS } from "./riskDomainsV6";
import { RSS_COMPONENT_EVIDENCE } from "./smcrEvidence";
import { WHAT_CHANGED_V6 } from "./whatChangedV6";
import { DERIVATION_STATIC_IDS } from "./derivations";

export function collectV6EvidenceRefs(): string[] {
  const refs = new Set<string>();

  for (const row of WHAT_CHANGED_V6) refs.add(row.evidenceRef);
