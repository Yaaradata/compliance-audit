/**
 * v6 "What Changed" rows — evidence-bound, with NO confidence percentage.
 *
...
export const WHAT_CHANGED_V6: ChangeRowV6[] = ROWS.map((row, i) => ({ row, i }))
...
export function buildStillnessRows(asOf: string = EVALUATED_AT): StillnessRowV6[] {
  const rows: StillnessRowV6[] = [];
