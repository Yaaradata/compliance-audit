...
export type V4ScreenId = "C1" | "C2" | "C3" | "C4" | "C5" | "C6";
...
export interface KeystoneDataV4 {
  company: {
    exciseBase: { amount: number; unit: string; tag: V4SourceTag };
    penaltyPct: { value: number; tag: V4SourceTag };
    totalTaxes: { amount: number; unit: string; tag: V4SourceTag };
    fy2026Revenue: { amount: number; unit: string; tag: V4SourceTag };
    capacityHL: { low: number; high: number; unit: string; tag: V4SourceTag };
    exposureBand: { low: number; high: number; unit: string; tag: V4SourceTag };
  };
  reconciliation: {
    streams: ReconStream[];
    nodeState: NodeState;
    ...
    detection: { value: string; tag: V4SourceTag };
  };
  batch: {
    id: string;
    src: string;
    panel: [string, QcStatus][];
    abv: { lab: number; label: number; excise: number; delta: number; tag: V4SourceTag };
    dutyAtStake: { amount: number; unit: string; tag: V4SourceTag; basis: string };
  };
  receivables: {
    fy2025: number;
    fy2026: number;
    ...
    eclAgeing: V4SourceTag;
    distributorPoints: { low: number; high: number; tag: V4SourceTag };
    ageing: AgeingBucket[];
    src: string;
  };
  loads: DispatchLoad[];
  riskMatrix: {
    rows: RiskMatrixRow[];
    escalation: { steps: string[]; tag: V4SourceTag };
  };
  ...
  boardReport: {
    ...
    committees: {
      ...
      name: string;
      remit: string;
      remitTag: V4SourceTag;
      composition: string[];
      meetingCount: null;
    }[];
    complianceTable: { ruleRef: string; requirement: string; exceptionRef: string }[];
    assurance: { companiesAct: string; thanks: string; tag: V4SourceTag };
  };
