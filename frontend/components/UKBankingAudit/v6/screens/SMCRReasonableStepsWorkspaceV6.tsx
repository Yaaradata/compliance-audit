...
} from '@/components/UKBankingAudit/v6/smcr';
...
import { recordAcknowledgement } from '@/lib/ukbankingaudit/v6/dispositions';
import { v6RefKind } from '@/lib/ukbankingaudit/v6/refRouter';

export function SMCRReasonableStepsWorkspaceV6({ variant = 'v2', selectedSMFId, setSelectedSMFId, smfTrails, pendingDecisionId, setPendingDecisionId, decisionRationale, setDecisionRationale, captureSMFDecision, openDrawer, setActiveScreen, setSelectedPackId }) {
  const ui = getUkAuditUi(variant === 'v4' || variant === 'v6' ? 'v3' : variant);
  ...
  const openEvidence = useCallback(
    (ref: string) => {
      openDrawer?.(v6RefKind(ref), ref, 'smcrWorkspace');
    },
    [openDrawer],
  );
