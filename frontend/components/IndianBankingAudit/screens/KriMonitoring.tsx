...
export function KriMonitoring({ openDrawer, drillFromDrawer }: { openDrawer: OpenDrawer; drillFromDrawer: DrillFromDrawer }) {
          ...
          <ol className="list-decimal space-y-2 pl-4 text-xs text-slate-800">
            {deteriorators.map(({ k, trend }) => (
              <li key={k.kri_id}>
                ...
                <div className="text-[10px] text-slate-500">Δ {trend.toFixed(2)}</div>
              </li>
            ))}
          </ol>
        </SectionCard>
...
function KriCard({
  ...
  const valDisp =
    kri.unit === 'ratio' && latest ? latest.value.toFixed(3) : latest != null ? `${latest.value}` : '—';
