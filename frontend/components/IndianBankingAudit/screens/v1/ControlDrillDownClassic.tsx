...
export function ControlDrillDownClassic({
  selectedControlId,
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_10.5rem]">
          <div className="min-w-0">
            <div className="grid grid-cols-1 gap-x-8 gap-y-3 sm:grid-cols-2">
              ...
              <MetadataStat label="Linked obligations" value={ctrl.linked_obligations.join(', ') || 'ΓÇö'} mono />
              <MetadataStat label="Linked risks" value={ctrl.linked_risks.join(', ') || 'ΓÇö'} mono />
            </div>
          </div>
      ...
      {tab === 'overview' && (
        <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,1fr)_17.5rem]">
          ...
          <div className="min-w-0 space-y-3">
            ...
            >
              Run population test for this control ΓåÆ
            </button>
          </div>
        </div>
      )}
      ...
      {tab === 'population' && (
        <SectionCard title={`Population grid (${instances.length} ControlInstances)`} subtitle="Click any row ΓåÆ D-01 source lineage drawer">
              ...
              <tbody>
                {instances.map((ci) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5 text-right text-[10px] text-slate-600">{ci.latency_ms != null ? `${(ci.latency_ms / 1000).toFixed(1)}s` : 'ΓÇö'}</td>
                    <td className="px-2 py-1.5 text-[10px] text-slate-600 line-clamp-1">{ci.fail_reason || ci.evidence_gap_reason || ci.data_gap_reason || 'ΓÇö'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
