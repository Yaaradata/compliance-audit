...
import { TAG_BADGE, TAG_BORDER } from './insightTokens';
...
/** Domain-scoped AI Summary Wall — curated + case-derived insights, ranked by confidence. */
export function JourneyAiSummaryWall({ insights, domainName, updated = '05:15 IST', onOpenCase }: Props) {
  const queue = insights.filter((i) => i.severity !== 'info').length;

  return (
    <aside
      aria-label="AI Summary Wall"
      className="flex h-full min-h-[320px] flex-col overflow-hidden rounded-xl border border-slate-200 bg-slate-50/70 shadow-sm"
    >
