...
import type { ValueChainHeroMock } from "./value-chain-types";
  ...
  return (
    <div className="relative h-2.5 w-full overflow-hidden rounded-full bg-[var(--muted)] ring-1 ring-[var(--border)]">
      ...
      <div
        ...
        title="Supply chain vs use-phase boundary"
      />
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className={`text-[11px] font-bold uppercase tracking-[0.14em] ${isUp ? "text-[var(--primary)]" : "text-rose-700 dark:text-rose-300"}`}>{title}</p>
          <p className="mt-1 text-xs text-[var(--foreground-muted)]">{flowLabel}</p>
        </div>
        <div className={`rounded-xl border px-3 py-2 text-right shadow-sm ${isUp ? "border-[var(--primary)]/20 bg-[var(--primary-muted)]/15" : "border-rose-500/20 bg-rose-500/10"}`}>
          <p className={`text-2xl font-bold tabular-nums leading-none sm:text-3xl ${isUp ? "text-[var(--primary)]" : "text-rose-700 dark:text-rose-300"}`}>{pct}%</p>
      ...
      <dl className="mt-4 grid grid-cols-2 gap-2">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)]/60 px-3 py-2">
          ...
          <dd className="mt-0.5 font-mono text-sm font-bold tabular-nums">{fmtIN(tco2e)} tCO₂e</dd>
        </div>
        <div className="rounded-lg border border-dashed border-[var(--border)] bg-[var(--muted)]/30 px-3 py-2">
          ...
          <dd className="mt-0.5 text-xs font-medium">{categoryHint}</dd>
        </div>
      </dl>
      <p className="mt-4 text-[11px] font-semibold uppercase tracking-wide text-[var(--foreground-muted)]">{entityLabel}</p>
      <ul className="mt-2 flex flex-col gap-2">
        {entities.map((e) => {
          const key = `${lane}:${e.name}`;
          return (
            <li key={key}>
              <EntityChip name={e.name} note={e.note} expanded={expandedKey === key} onToggle={() => onExpand(expandedKey === key ? "" : key)} accent={lane} />
            </li>
          );
        })}
      </ul>
      <button
        ...
        className={`mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl border px-3 py-2.5 text-xs font-semibold text-white shadow-sm ${
          isUp ? "border-[var(--primary)]/30 bg-[var(--primary)] hover:bg-[var(--primary-hover)]" : "border-rose-600/30 bg-rose-600 hover:bg-rose-700"
        }`}
      >
        {exploreLabel} ↓
      </button>
...
export function ValueChainHero({ legalName, hero }: { legalName: string; hero: ValueChainHeroMock }) {
  const [activeLane, setActiveLane] = useState<HeroLane | null>(null);
  const [expandedKey, setExpandedKey] = useState<string | null>(null);
  const onExpand = useCallback((key: string) => setExpandedKey(key || null), []);

  return (
    <div className="overflow-hidden">
      <div className="border-b border-[var(--border)] bg-[var(--muted)]/25 px-5 py-5 sm:px-6">
        <h1 className="text-lg font-bold tracking-tight sm:text-xl">
          {legalName} <span className="text-[var(--foreground-muted)]">({hero.companyTicker})</span>
        </h1>
        <p className="mt-1 text-sm text-[var(--foreground-muted)]">Value-chain lens — supply chain & inbound vs use phase & end-of-life (FY25 mock).</p>
        <div className="mt-4">
      ...
      <div className="grid min-h-[480px] grid-cols-1 md:grid-cols-2">
        <LanePanel
          lane="upstream"
          title="Upstream"
          flowLabel={hero.flowLeftLabel}
          pct={hero.upstreamPctScope3}
          tco2e={hero.upstreamTCO2e}
          tco2eLabel="Supply chain & inbound"
          entities={hero.upstreamEntities}
          entityLabel="Key tier-1 suppliers"
          categoryHint="Cat 1–7"
          active={activeLane === "upstream"}
          expandedKey={expandedKey}
          onExpand={onExpand}
          onFocus={() => setActiveLane("upstream")}
          onBlur={() => setActiveLane(null)}
          exploreTargetId="vc-upstream"
          exploreLabel="Explore upstream detail"
        />
        <LanePanel
          lane="downstream"
          title="Downstream"
          flowLabel={hero.flowRightLabel}
          pct={hero.downstreamPctScope3}
          tco2e={hero.downstreamTCO2e}
          tco2eLabel="Use phase & EOL"
          entities={hero.downstreamEntities}
          entityLabel="Vehicle lines & markets"
          categoryHint="Cat 9, 11, 12"
          active={activeLane === "downstream"}
          expandedKey={expandedKey}
          onExpand={onExpand}
          onFocus={() => setActiveLane("downstream")}
          onBlur={() => setActiveLane(null)}
          exploreTargetId="vc-downstream"
