...
import type {
  ValueChainModelDrill,
  ValueChainCategoryDrill,
  ValueChainSegmentDrill,
  ValueChainSectorDrill,
  ValueChainSupplierDrill,
} from "./value-chain-types";

export type VcDrill =
  | { kind: "category"; title: string; drill: ValueChainCategoryDrill }
  | { kind: "supplier"; title: string; drill: ValueChainSupplierDrill }
  | { kind: "segment"; title: string; drill: ValueChainSegmentDrill }
  | { kind: "sector"; title: string; drill: ValueChainSectorDrill }
  | { kind: "model"; title: string; drill: ValueChainModelDrill };
...
export function ValueChainDrillBody({ drill }: { drill: VcDrill }): ReactNode {
  if (drill.kind === "category") {
    ...
    return (
      <div className="space-y-5 text-sm">
        ...
        <Section title="Plant / location split (mock)">
  ...
  if (drill.kind === "supplier") {
    ...
    return (
      <div className="space-y-5 text-sm">
        ...
        <Section title="Emission factor source">
              ...
              <tbody>
                {d.topConcentrations.map((r) => (
                  <tr key={r.name} className="border-t border-[var(--border)]">
                    ...
                    <td className="px-3 py-2 text-right font-mono tabular-nums">{r.units.toLocaleString("en-IN")}</td>
                    <td className="px-3 py-2 text-right font-mono tabular-nums">{r.tco2e.toLocaleString("en-IN")}</td>
                    <td className="px-3 py-2 text-[var(--foreground-muted)]">{r.note ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  if (drill.kind === "sector") {
    ...
    return (
      <div className="space-y-5 text-sm">
        ...
        <Section title="Representative models">
          <div className="overflow-x-auto rounded-lg border border-[var(--border)]">
            <table className="w-full border-collapse text-xs">
              <thead className="bg-[var(--muted)]/50 text-left text-[10px] font-semibold uppercase tracking-wide text-[var(--foreground-muted)]">
                <tr>
              ...
              <tbody>
                {d.topModels.map((r) => (
                  <tr key={r.name} className="border-t border-[var(--border)]">
                    <td className="px-3 py-2 font-medium text-[var(--foreground)]">{r.name}</td>
                    <td className="px-3 py-2 text-right font-mono tabular-nums">{r.units.toLocaleString("en-IN")}</td>
                    <td className="px-3 py-2 text-center font-mono font-semibold">{r.intensity}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  if (drill.kind !== "model") return null;
