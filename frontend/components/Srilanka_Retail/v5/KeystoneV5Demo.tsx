...
import { createInitialStore } from "@/lib/Srilanka_Retail/v5/seed";
import type { KeystoneDataV5, V5ScreenId } from "@/lib/Srilanka_Retail/v5/types";
import {
  KeystoneV5ThemeProvider,
  useKeystoneV5Colors,
  useKeystoneV5ThemeMode,
...
import { SidebarNav, V5_NAV } from "./shell/SidebarNav";

function KeystoneV5DemoInner({
  mode,
  onToggleTheme,
}: {
  mode: ReturnType<typeof useKeystoneV5ThemeMode>[0];
  onToggleTheme: () => void;
}) {
  const C = useKeystoneV5Colors();
  const [store, setStore] = useState<KeystoneDataV5>(() => createInitialStore());
  const [screen, setScreen] = useState<V5ScreenId>("C1");
  ...
  const active = V5_NAV.find((n) => n[0] === screen)!;
...
export default function KeystoneV5Demo() {
  const [mode, setMode] = useKeystoneV5ThemeMode();

  return (
    <KeystoneV5ThemeProvider mode={mode}>
      <KeystoneV5DemoInner
        mode={mode}
