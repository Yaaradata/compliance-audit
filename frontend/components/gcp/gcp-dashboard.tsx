...
import { getGcpEvidenceContent } from "@/lib/gcp-api";

interface GcpDashboardProps {
  runs: GcpRun[];
  evidenceRows: GcpEvidenceRow[];
  evidenceCount: number;
  controlIdsWithEvidence: string[];
  onFetchEvidence: () => void;
  onOpenRunComparisor: (row: GcpEvidenceRow) => void;
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between md:gap-6">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm leading-snug" style={{ color: "var(--foreground-muted)" }}>
              Compliance evidence at a glance — collect and view Google Cloud evidence for SWIFT controls (ADC + project
              from backend configuration).
            </p>
          </div>
          <div className="flex w-full flex-wrap items-center justify-end gap-2 md:w-auto md:shrink-0">
            <Link href="/gcp" className={`${awsButtonSecondaryClass} min-h-[42px] px-3 sm:px-4`}>
              Connect
            </Link>
      ...
      {runs.length > 0 && (
        ...
        >
          <RunHistoryVisualsPlotly
            ...
            fetchEvidenceContent={(id, cycleId) => getGcpEvidenceContent(id, cycleId)}
          />
        </section>
      )}
      ...
      {activeSection === "evidence" && (
        <section id="dashboard-evidence" aria-label="Evidence section" className="w-full">
          <AwsEvidenceTable
            ...
            fetching={fetching}
            fetchButtonLabel="Fetch GCP evidence"
          />
        </section>
      )}
    </div>
